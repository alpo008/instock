<?php
/**
 * Message translations.
 *
 * This file contains the localizable messages extracted from project code.
 * You may modify this file by translating the extracted messages.
 *
 * Each array element represents the translation (value) of a message (key).
 * If the value is empty, the message is considered as not translated.
 * Messages that no longer need translation will have their translations
 * enclosed between a pair of '@@' marks.
 *
 * Message string can be used with plural forms format. Check i18n section
 * of the guide for details.
 *
 * NOTE: this file must be saved in UTF-8 encoding.
 */
return [
    '{attribute} must be equal to "{compareValueOrAttribute}".' => 'Значение «{attribute}» должно быть равно «{compareValueOrAttribute}».',
    '{attribute} must be greater than "{compareValueOrAttribute}".' => 'Значение «{attribute}» должно быть больше значения «{compareValueOrAttribute}».',
    '{attribute} must be greater than or equal to "{compareValueOrAttribute}".' => 'Значение «{attribute}» должно быть больше или равно значения «{compareValueOrAttribute}».',
    '{attribute} must be less than "{compareValueOrAttribute}".' => 'Значение «{attribute}» должно быть меньше значения «{compareValueOrAttribute}».',
    '{attribute} must be less than or equal to "{compareValueOrAttribute}".' => 'Значение «{attribute}» должно быть меньше или равно значения «{compareValueOrAttribute}».',
    '{attribute} must not be equal to "{compareValueOrAttribute}".' => 'Значение «{attribute}» не должно быть равно «{compareValueOrAttribute}».',
    '(not set)' => '(не задано)',
    'Access denied' => 'Доступ запрещен',
    'Active' => 'Активен',
    'Addition' => 'Добавление',
    'Add material' => 'Добавить материал',
    'Add stock place' => 'Добавить складское место',
    'Add user' => 'Добавить пользователя',
    'Alias' => 'Обозначение',
    'Admin page' => 'Страница администратора',
    'An internal server error occurred.' => 'Возникла внутренняя ошибка сервера.',
    'Are you sure you want to block this user' => 'Вы уверены, что хотите заблокировать данного пользователя',
    'Are you sure you want to delete this item?' => 'Вы уверены, что хотите удалить этот элемент?',
    'Applying to column \'ref\'' => 'Применяется к столбцу \'Текущее количество\'',
    'Auth key' => 'Ключ авторизации',
    'Available at' => 'В наличии на месте',
    'Backups' => 'Резервные копии',
    'Bad columns configuration' => 'Неверные параметры таблицы',
    'Bad parameters' => 'Неверные параметры',
    'Bad request' => 'Неверный запрос',
    'Block' => 'Заблокировать',
    'Cancel' => 'Отмена',
    'Can not accept negative quantity' => 'Текущее количество не может быть отрицательным',
    'Changed' => 'Изменено',
    'Close form' => 'Закрыть форму',
    'Comments' => 'Комментарии',
    'Correction' => 'Корректирующая запись',
    'Correction operation' => 'Корректирующая запись',
    'Correction reason' => 'Причина корректировки',
    'Create debit operation' => 'Создать запись о приходе',
    'Create correction operation' => 'Создать корректирующую запись',
    'Create credit operation' => 'Создать запись о расходе',
    'Created at' => 'Дата создания',
    'Created by' => 'Кто добавил',
    'Credit' => 'Расход',
    'Credit operation' => 'Расходная операция',
    'Debit' => 'Приход',
    'Debit operation' => 'Приходная операция',
    'Delete' => 'Удалить',
    'Delete group' => 'Удалить машину',
    'Delete material' => 'Удалить материал',
    'Delete stock place' => 'Удалить складское место',
    'Delete type' => 'Удалить тип',
    'Description' => 'Описание',
    'Details' => 'Подробно',
    'Disabled' => 'Блокирован',
    'Download' => 'Сохранить',
    'Duplicated ref action' => 'Действие при совпадении SAP кода',
    'Edit' => 'Изменить',
    'Edit material card' => 'Редактировать карту материала',
    'Edit stock place' => 'Редактировать складское место',
    'Enter new machine name' => 'Введите обозначение новой машины',
    'Enter new type name' => 'Введите обозначение нового типа материалов',
    'Excel file columns' => 'Столбцы в файле Excel',
    'Export' => 'Экспорт',
    'Excel reading error' => 'Ошибка чтения файла Excel',
    'Export table to Excel' => 'Экспортировать таблицу в Excel',
    'from cell' => 'из ячейки',
    'File' => 'Файл',
    'Fill only if you want to change old one' => 'Заполняется только в случае изменения старого пароля',
    'For this material quantity can be only integer' => 'Для данного материала дробные количества неприменимы',
    'From stock place {stock} were taken {qty} of material {material}' =>
        'Со складского места {stock} был взят материал "{material}" в количестве {qty}',
    'From where' => 'Откуда',
    'Full name' => 'Полное имя',
    'General purpose' => 'Общего назначения',
    'Greater than max quantity' => 'Больше максимального',
    'Group' => 'Машина',
    'Groups' => 'Машины',
    'History' => 'История',
    'How many' => 'Сколько единиц',
    'Ignore first row' => 'Игнорировать первую строку',
    'Inspection stock' => 'Инспекция - склад',
    'ID' => 'ИД',
    'Import' => 'Импорт',
    'Imported rows' => 'Импортировано строк',
    'Import in progress' => 'Импорт данных',
    'Import materials from file' => 'Импорт материалов из файла',
    'Import structure' => 'Структура импорта',
    'Impossible to delete this user' => 'Невозможно заблокировать пользователя',
    'Impossible to restore this user' => 'Невозможно разблокировать пользователя',
    'Incorrect username or password' => 'Неверно указано имя пользователя или пароль',
    'Kg' => 'Кг',
    'Last edition' => 'Последнее изменение',
    'Latin letters and digits' => 'Буквы латинского алфавита и цифры',
    'Latin letters, digits, special symbols' => 'Буквы латинского алфавита, цифры, спецсимволы',
    'Less than min quantity' => 'Меньше минимального',
    'login' => 'Логин',
    'List of materials' => 'Перечень материалов',
    'Login' => 'Авторизация',
    'Logout' => 'Выход',
    'Main' => 'Главная',
    'Material' => 'Материал',
    'Materials' => 'Материалы',
    'Materials groups list edition' => 'Редактирование списка машин',
    'Materials types' => 'Типы материалов',
    'Materials types list edition' => 'Редактирование списка типов материалов',
    'Materials export' => 'Экспорт материалов',
    'Material card edition: {name}' => 'Редактирование карты материала: {name}',
    'Material destination' => 'На что израсходован',
    'Material movement' => 'Перемещение материала',
    'Material name' => 'Описание материала',
    'Material not found' => 'Материал не найден',
    'Material source' => 'Откуда получен',
    'Meters' => 'м',
    'Move material' => 'Переместить материал',
    'Name' => 'Имя',
    'Max qty' => 'Максимальное количество',
    'Min qty' => 'Критически низкое количество',
    'New stock operation' => 'Создать новую запись',
    'New password' => 'Новый пароль',
    'No group' => 'Без группы',
    'Not set' => 'Не задано',
    'Only excel files are allowed' => 'Только файл в формате Excel',
    'Operation error. Please contact administrator' =>
        'Ошибка при выполнении операции. Пожалуйста, сообщите об этом администратору',
    'Pairs' => 'Пар',
    'Operations' => 'Операции',
    'Operation details' => 'Подробности операции',
    'Operation type' => 'Тип операции',
    'Password' => 'Пароль',
    'Pcs' => 'Шт',
    'Please fill out the following fields' => 'Пожалуйста, заполните форму',
    'Photo' => 'Фото',
    'Position' => 'Должность',
    'Qty' => 'Текущее количество',
    'Quantity' => 'Количество',
    'Quantity in range' => 'В диапазоне',
    'Quantity can not be greater then exists on place' => 'На выбранном месте нет такого количества материала',
    'Quantity can not be less than' => 'Количество не может быть меньше',
    'Quantity update error' => 'Ошибка изменения тепкущего количества',
    'rest' => 'остаток',
    'Ref' => 'Номер САП',
    'Remember me' => 'Запомнить меня',
    'Responsible person' => 'Ответственное лицо',
    'Restore' => 'Разблокировать',
    'Role' => 'Роль',
    'Russian letters only' => 'Только русские буквы',
    'Save' => 'Сохранить',
    'Save database value' => 'Сохранить количество из БД',
    'Save file value' => 'Сохранить количество из файла',
    'Server page' => 'Сервер <br> Уфа-Электрон',
    'Settings' => 'Настройки',
    'Source and target places can not be the same' => 'Старое и новое место хранения не должны совпадать',
    'Source or destination' => 'Источник / получатель',
    'Start to type ref or name' => 'Начните вводить SAP или наименование',
    'Status' => 'Статус',
    'Stock operations' => 'Приходно-расходные операции',
    'Stock place' => 'Складское место',
    'Stock places' => 'Складские места',
    'Stock place addition' => 'Добавление складского места',
    'Stock place not found' => 'Складское место не найдено',
    'Sum database and file values' => 'Суммировать количества из БД и из файла',
    'Surname' => 'Фамилия',
    'Table' => 'Таблица',
    'Take' => 'Взять',
    'There are no urgent messages' => 'Срочных сообщений нет',
    'The rest is only {max} at stock place' => 'Из данной ячейки можно взять не более {max}',
    'To where' => 'Куда',
    'Type' => 'Тип',
    'Unit' => 'Единица измерения',
    'Update' => 'Редактировать',
    'Updating' => 'Редактирование',
    'Updated at' => 'Дата последнего изменения',
    'Updated by' => 'Кто отредактировал',
    'Update stock: {name}' => 'Редактирование: {name}',
    'Update user: {name}' => 'Редактирование: {name}',
    'Upload the file' => 'Необходимо загрузить файл',
    'Users' => 'Пользователи',
    'User with role {role} will be restored' => 'Пользователь будет восстановлен с ролью {role}',
    'User name' => 'Имя пользователя',
    'View' => 'Просмотр',
    'View material' => 'Просмотр материала',
    'View urgent messages' => 'Просмотр срочных сообщений',
    'View warning messages' => 'Просмотр предупреждений',
];
